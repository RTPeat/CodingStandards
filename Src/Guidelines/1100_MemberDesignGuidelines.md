...
### <a name="ft1100"></a> Allow properties to be set in any order (FT1100) ![](images/1.png)
...
### <a name="ft1105"></a> Use a method instead of a property (FT1105) ![](images/3.png)
...
### <a name="ft1110"></a> Don't use mutual exclusive properties (FT1110) ![](images/1.png)

Having properties that cannot be used at the same time typically signals a type that represents two conflicting concepts. Even though those concepts may share some of their behaviour and states, they obviously have different rules that do not cooperate.
...
### <a name="ft1115"></a> A method or property should do only one thing (FT1115) ![](images/1.png)

Similarly to rule FT1000, a method should have a single responsibility.

### <a name="ft1125"></a> Don't expose stateful objects through static members (FT1125) ![](images/2.png)

A stateful object is an object that contains many properties and lots of behaviour behind it. If you expose such an object through a static property or method of some other object, it will be very difficult to refactor or unit test a class that relies on such a stateful object. In general, introducing a construction like that is a great example of violating many of the guidelines of this chapter.
...
### <a name="ft1130"></a> Return an `IEnumerable<T>` or `ICollection<T>` instead of a concrete collection class (FT1130) ![](images/2.png)
...
**Note:** If you're using .NET 4.5 or later, you can also use `IReadOnlyCollection<T>`, `IReadOnlyList<T>` or `IReadOnlyDictionary<TKey, TValue>`.

### <a name="ft1135"></a> Properties, methods and arguments representing strings or collections should never be `null` (FT1135) ![](images/1.png)
...
### <a name="ft1137"></a> Define parameters as specific as possible (FT1137) ![](images/2.png)
...
### <a name="ft1140"></a> Consider using domain-specific value types rather than primitives (FT1140) ![](images/3.png)
