...
### <a name="ft1800"></a> Consider using `Any()` to determine whether an `IEnumerable<T>` is empty (FT1800) ![](images/3.png)
...
**Note:** If you return an `IEnumerable<T>` to prevent editing from outside the owner as explained in FT1130, and you're developing in .NET 4.5 or higher, consider the new read-only classes.

### <a name="ft1820"></a> Only use `async` for low-intensive long-running activities (FT1820) ![](images/1.png)
...
### <a name="ft1825"></a> Prefer `Task.Run` for CPU-intensive activities (FT1825) ![](images/1.png)
...
### <a name="ft1830"></a> Beware of mixing up `await`/`async` with `Task.Wait` (FT1830) ![](images/1.png)
`await` does not block the current thread but simply instructs the compiler to generate a state-machine. However, `Task.Wait` blocks the thread and may even cause deadlocks (see FT1835).

### <a name="ft1835"></a> Beware of `async`/`await` deadlocks in single-threaded environments (FT1835) ![](images/1.png)
